...
module FinapiApiClient
  ...
  class EditBankConnectionParams
    ...
    # Set of login credentials that you want to edit. Must be passed in combination with the 'interface' field. The labels that you pass must match with the login credential labels that the respective interface defines. If you want to clear the stored value for a credential, you can pass an empty string (\"\") as value.<br><br>NOTE: When you pass this field, then the fields 'bankingUserId','bankingCustomerId' and 'bankingPin' will be ignored.
    ...
    def hash
      [banking_user_id, banking_customer_id, banking_pin, default_two_step_procedure_id, name, interface, login_credentials].hash
    end
